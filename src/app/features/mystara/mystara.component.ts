...
@Component({
  selector: 'app-mystara',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './mystara.component.html',
  styleUrls: ['./mystara.component.scss']
})
export class MystaraComponent implements OnInit, OnDestroy {
  ...
  viewport = computed(() => {
    const s = this.state();
    if (!s) return null;
    return this.mystaraService.getViewport(s, VP_W, VP_H);
  });
  ...
  ngOnInit(): void {
    this.state.set(this.mystaraService.initMystara());
    this.setStatus('🌀 You emerge into Mystara — the Known World!');
  }
  ...
  private step(dx: number, dy: number): void {
    ...
    const event = this.mystaraService.movePlayer(s, dx, dy);
    this.state.set({ ...s });
    ...
    switch (event.type) {
      case 'blocked':
        ...
        break;
      case 'enter-portal2':
        this.setStatus('🌀 The portal shimmers... returning to Westeros...');
  ...
  cellStyle(cell: OverworldCell | null, vpX: number, vpY: number): Record<string, string> {
    if (vpX === this.playerVPX && vpY === this.playerVPY) {
      return { color: '#ffffff', 'background-color': '#003322' };
    }
